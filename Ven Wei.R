...
sampsize <- 194   # trial sample size. # change for each trial &&&
pertimemax <- 38    # max followup time  # change for each trial &&&
perrectimemax <- 18   # length of recruitment period # change for each trial &&&
timehorizon <- 240    # time horizon of economic model # change for each trial based on time horizon &&&
lambdarec <- 0.1      # change for each trial, exponential parameter for recruitment &&&
lambdacens <- 0.00222 # lambda of low underlying censoring rate
dpertu <- read.delim("//gobo/USER40/u/u1474467/Documents/PHD/AIC paper/Trials/digitised data/IPD ven.txt") #change &&&
...
closeAllConnections()
console_output <- file("Ven_Wei_Log.txt") ## change &&&
...
write.csv(results,"Ven_Wei_Results.csv")
