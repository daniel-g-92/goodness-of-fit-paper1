...
exper <- flexsurvreg(Surv(V1, V2) ~ 1, data=dpertu, dist = "exp") #### change here if alternate dist and lines 49-50 + 210, 211 215 &&&
...
closeAllConnections()
console_output <- file("Pembro_Exp_Log.txt") ## change &&&


results$truemean <- rmst_exp(timehorizon, rate = exper$res[1,1])    #### change if using alternate distribution &&&
results$truedist <- "Exp"                             #### change if using alternate distribution &&&
...
for (k in 1:nsim){ 
  ...
  results[k,142] <- exper$res[1,1]
  timecens <- rexp(sampsize, lambdacens)
  
  #### starting with data setup ####
  
  # set up for  run 1 - no censoring
  survperexp <- data.frame(rexp(sampsize, rate = exper$res[1,1]), recper,timecens) # change for each distribution
...
write.csv(results,"Pembro_Exp_Results.csv")
