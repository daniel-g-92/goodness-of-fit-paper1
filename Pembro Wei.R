...
sampsize <- 270   # trial sample size. # change for each trial &&&
pertimemax <- 18    # max followup time  # change for each trial &&&
perrectimemax <- 12   # length of recruitment period # change for each trial &&&
timehorizon <- 420    # time horizon of economic model # change for each trial based on time horizon &&&
lambdarec <- 0.2      # change for each trial, exponential parameter for recruitment &&&
lambdacens <- 0.002149 # lambda of low underlying censoring rate
dpertu <- read.delim("//gobo/USER40/u/u1474467/Documents/PHD/AIC paper/Trials/digitised data/IPD pembro.txt") #change &&&



# fit required model to obtain parameters
fit <- surv_fit(Surv(V1, V2) ~ 1, data=dpertu )
exper <- flexsurvreg(Surv(V1, V2) ~ 1, data=dpertu, dist = "weibull") #### change here if alternate dist and lines 49-50 + 210, 211 215 &&&
...
closeAllConnections()
console_output <- file("Pembro_Wei_Log.txt") ## change &&&


results$truemean <- rmst_weibull(timehorizon,shape = exper$res[1,1], scale = exper$res[2,1])     #### change if using alternate distribution &&&
results$truedist <- "Wei"                             #### change if using alternate distribution &&&
...
for (k in 1:nsim){ 
  ...
  survperexp <- data.frame(rweibull(sampsize, shape = exper$res[1,1], scale = exper$res[2,1]), recper,timecens) # change for each distribution
...
write.csv(results,"Pembro_Wei_Results.csv")
