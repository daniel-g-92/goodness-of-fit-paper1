...
sampsize <- 227   # trial sample size. # change for each trial &&&
pertimemax <- 46    # max followup time  # change for each trial &&&
perrectimemax <- 23   # length of recruitment period # change for each trial &&&
timehorizon <- 180    # time horizon of economic model # change for each trial based on time horizon &&&
lambdarec <- 0.1      # change for each trial, exponential parameter for recruitment &&&
lambdacens <- 0.00233 # lambda of low underlying censoring rate
dpertu <- read.delim("//gobo/USER40/u/u1474467/Documents/PHD/AIC paper/Trials/digitised data/Daco IPD OS.txt") #change &&&



# fit required model to obtain parameters
fit <- surv_fit(Surv(V1, V2) ~ 1, data=dpertu )
exper <- flexsurvreg(Surv(V1, V2) ~ 1, data=dpertu, dist = "lnorm") #### change here if alternate dist and lines 49-50 + 210, 211 215 &&&
...
closeAllConnections()
console_output <- file("Daco_LogN_Log.txt") ## change &&&


results$truemean <- rmst_lnorm(timehorizon,meanlog = exper$res[1,1], sdlog = exper$res[2,1])     #### change if using alternate distribution &&&
results$truedist <- "LogN"                             #### change if using alternate distribution &&&
...
for (k in 1:nsim){ 
  ...
  survperexp <- data.frame(rlnorm(sampsize, meanlog = exper$res[1,1], sdlog = exper$res[2,1]), recper, timecens) # change for each distribution
...
write.csv(results,"Daco_LogN_Results.csv")
